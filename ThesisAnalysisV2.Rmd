...
#predictors  <- "~ 1 + nr_imputed + avg_BIC + PCCs_prop_empty + diff_norm_PA + diff_norm_NA + diff_norm_Stress + diff_norm_Tasks"
...
```{r plotNetwdescr}
comparisons %>%
ggplot( aes(x = PCCs_prop_empty, y = PCCs_cor, size = PCCs_prop_equ_sign_non0 , color = PCCs_Jaccard)) + 
  geom_point(alpha = 2) +
...
```{r regressionResults}
"Did anything else happen to you since this time yesterday that most people would consider stressful?"summary(fit.Cor) # swl + neoc
summary(fit.CorNon0) # - 
