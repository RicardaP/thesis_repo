...
for (i in 1:N){
    temp <- filter(dat, participantID==i, day.response %in% 51:100)
    for ( v in seq_along(emaLabels)){
...
for (i in 1:N) { # per individual i
  for (v in seq_along(emaLabels)){ # per variable v
    formula <- as.formula(paste0(emaLabels[v], " ~ 1 + day.response")) # define formula for linear trend
      trend_v <- lm(formula, data = dat[dat$participantID == i, ]) # fit model per person per var
      dat_detrended[dat_detrended$participantID == i, 5+v] <- residuals(trend_v) # save detrended var
  }
}; rm(i, v, formula, trend_v)

dat <- dat_detrended
# plot(dat_detrended[dat_detrended$participantID == i, 5+v])
# plot(residuals(trend_v))
# NOTE: 
...
1:3 + 1:3 / 2
rowMeans(cbind(1:3, 1:3))

# To do ask EIKO: should I use avearge scores instead? more common I guess
dat$PosAffect_avg <- rowMeans( cbind( 
  dat$alert, dat$active, dat$attentive, dat$determined,  dat$inspired ))
dat$NegAffect_avg <- rowMeans( cbind( 
  dat$afraid, dat$nervous, dat$hostile, dat$ashamed, dat$upset ))
dat$StressSev_avg <- rowMeans( cbind( 
  dat$severe1, dat$severe2, dat$severe3, dat$severe4, dat$severe5, dat$severe6, dat$severe7 ))
...
networks <- data_frame(
  ...
  Rank1 = 
  Rank2 =
  SumMissingT1 # sum of imputed data points in T1
  SumMissingT2 # sum of imputed data points in T2
  b_trend =  # beta estimate of detrending effects?
)
...
#  extract two highest ranking DPDS variables per person. Note: higher rank stat = better
temp <- filter(log_loops, log_loops$ID == i, log_loops$emaLabels %in% grep("dpds", log_loops$emaLabels, value = TRUE))
ranked <- temp[order(temp$rank_stat, decreasing = T), ]
Rank1 <- ranked[1, 3]
Rank2 <- ranked[2, 3]
dat_T1 <- select(dat_T1, day.response, PosAffect_sum, NegAffect_sum, StressSev_sum, tasks, Rank1, Rank2 ) # To do: include Rank1 and Rank2
dat_T2 <- select(dat_T2, day.response, PosAffect_sum, NegAffect_sum, StressSev_sum, tasks, Rank1, Rank2 ) # To do: include Rank1 and Rank2
# fit network t2
netw_T1 <- graphicalVAR(dat_T1, beepvar = "day.response")
netw_T2 <- graphicalVAR(dat_T2, beepvar = "day.response")
# save network objects (for later)
# assign(paste0("ID",i,"_T1"), netw_T1) #4.4mb per netw
netw_T1$EBIC
netw_T1$gamma
netw_T1$N
netw_T1$labels
netw_T1$path
netw_T1$PCC # path coeff
netw_T1$kappa # unst
# netw_T1[1:7] # netw object without data and without all results
# netw_T2[1:7]
...
# cor(netw_T1$PCC, netw_T2$PCC)
# cor(vechs(netw_T1$PDC), vechs(netw_T2$PDC))
